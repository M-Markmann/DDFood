...
def trying_tester(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as Discounting:
        ...
        Discounting_trace = pm.sample(samples, chains=chains, cores=cores, tune=tune, progressbar=True,  init='adapt_diag',
                                         return_inferencedata = True,  discard_tuned_samples=False, idata_kwargs={"log_likelihood": True, })
        return Discounting_trace
            #nuts = {'target_accept': 0.75},
...
def testing_exponential(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as Exponential:
        k_mu = pm.Normal('K_mu', mu=-2.43, sigma=0.27, dims=['Subject'])

        k_sigma = pm.HalfNormal('K_sigma', sigma=0.072, dims=['Subject'])
        alpha_mu = pm.Uniform('alpha_mu', lower=0, upper=10000, dims=['Subject'], initval=np.repeat(1,n_subj))

        #alpha_sigma = pm.Uniform('alpha_sigma', lower=0, upper=1000, dims=['Subject'])

        error_mu = pm.Beta('error_mu',alpha=1.1, beta=10.9, dims=['Subject'])
        error_sigma = pm.Gamma('error_sigma', alpha=0.01, beta=0.01, dims=['Subject'], initval=np.repeat(1,n_subj))
        k = pm.Normal('k',  mu=k_mu, sigma=k_sigma, dims=['Subject'], initval=np.repeat(-1,n_subj)) #mu=k_mu, sigma=k_sigma,
        alpha = pm.HalfNormal('alpha', sigma=alpha_mu, dims=['Subject'], initval=np.repeat(1,n_subj)) #mu=alpha_mu, sigma=alpha_sigma, , shape=(n_subj,1)
        error = pm.Beta('error', alpha=(error_mu*(error_sigma-2)+1), beta=((1-error_mu)*(error_sigma-2)+1))
        # Subjective Values
        V_A = pm.Deterministic("V_A", A +pytensor.tensor.log10(k[:])*DA, dims=['Trial_NR','Subject'])
        Exponential.debug()
        V_B = pm.Deterministic("V_B", B*pm.math.exp(-1*pytensor.tensor.log10(k[:])*DB), dims=['Trial_NR','Subject'])
        Exponential.debug()
        ...
        #p_delayed = pm.Deterministic('p_delayed', ((1)/(1+pm.math.exp(-1*(V_B-V_A))))) #alpha[:]*
        #softmax_scaled =pm.Deterministic('softmax_scaled', 0.001 * 0.5 + ((1 - 0.001) * p_delayed))
        p_delayed = pm.Deterministic('p_delayed', ((1) / (1 + pm.math.exp(-1 * (V_B[:,:] - V_A[:,:])))))  # alpha[:]*
        Exponential.debug()
        #softmax_scaled = pm.Deterministic('softmax_scaled', 0.001 * 0.5 + ((1 - 0.001) * p_delayed))
        h = pm.Bernoulli('h', p=p_delayed, observed=R, dims=['Trial_NR',
                                                                  'Subject'])  # was logit_p #, shape=(n_subj,n_trials) , shape=(n_trials,n_subj)
        Exponential.debug()
        # Sample Binomial',n=(n_trials*n_subj)
        #log_P = normal_lcdf(0, 1, (V_B - V_A) / alpha[:])  # log(p)
        #log_1m_P = pm.math.log1mexp(-log_P)  # log(1-p)
        #logit_P = pm.Deterministic('logit_P', log_P - log_1m_P)
        # Observed
...
def testing_hyperbol(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as Hyperbol:
        k_mu = pm.Normal('K_mu', mu=0.01, sigma=4, dims=['Subject'])

        k_sigma = pm.Uniform('K_sigma', lower=0, upper=2, dims=['Subject'])
        ...
        p_delayed = pm.Deterministic('p_delayed', ((1)/(1+pm.math.exp(-1*(V_B-V_A))))) #alpha[:]*
        softmax_scaled =pm.Deterministic('softmax_scaled', 0.001 * 0.5 + ((1 - 0.001) * p_delayed))
        h = pm.Bernoulli('h', p=softmax_scaled, observed=R, dims=['Trial_NR','Subject']) # , dims=['Subject','Trial_NR','prob']) #*alpha[:]
        # Observed
...
def testing_hyperbol_scaling_delay(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as Hyperbol:
        ...
        s_sigma = pm.Uniform('s_sigma', lower=0, upper=3, dims=['Subject'])
...
def testing_hyperbol_scaling_both(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as Hyperbol:
        ...
        p_delayed = pm.Deterministic('p_delayed', ((1)/(1+pm.math.exp(-1*(V_B-V_A))))) #alpha[:]*
        softmax_scaled =pm.Deterministic('softmax_scaled', 0.001 * 0.5 + ((1 - 0.001) * p_delayed))
        h = pm.Bernoulli('h', p=softmax_scaled, observed=R, dims=['Trial_NR','Subject'])   # , dims=['Subject','Trial_NR','prob'])

  # , dims=['Subject','Trial_NR','prob']) #*alpha[:]
...
def testing_DRIFT(chains,samples,n_subj,A,B,DB,R,DA, cores, tune, i, dict, n_trials):
    with pm.Model(coords=dict) as DRIFT:
        ...
        parameters = pd.concat([parameters_driftD, parameters_driftR, parameters_driftI, parameters_driftT, parameters_scaling ], axis=1)#parameters_alpha
        parameters.columns = ["lower_bound_driftD", "higher_bound_driftD", "lower_bound_driftR",
                              "higher_bound_driftR",
                              "lower_bound_driftI", "higher_bound_driftI", "lower_bound_driftT", "higher_bound_driftT","lower_bound_scaling",
